...
//  8 themes • 7 languages (EN/FR/AR/ES/DE/TR/RU) • QR engine • tray •
//  settings dialog • login PIN • delete PIN • ZIP download • thumbnails •
//  resumable downloads • speed limit • auto-refresh • notes→clipboard •
//  camera upload • search/sort/grid • per-file QR/copy • traffic monitor •
//  GUI upload progress • log file • auto-start • single instance
...
namespace GoldShare
{
    ...
    public static class L
    {
        ...
        private static readonly Dictionary<string,string> En=MakeEn();
        private static readonly Dictionary<string,string> Fr=MakeFr();
        private static readonly Dictionary<string,string> Ar=MakeAr();
        private static readonly Dictionary<string,string> Es=MakeEs();
        private static readonly Dictionary<string,string> De=MakeDe();
        private static readonly Dictionary<string,string> Tr=MakeTr();
        private static readonly Dictionary<string,string> Ru=MakeRu();
        ...
        public static void Set(string code)
        {
            ...
            if(lang=="fr")dict=Fr; else if(lang=="ar")dict=Ar; else if(lang=="es")dict=Es;
            else if(lang=="de")dict=De; else if(lang=="tr")dict=Tr; else if(lang=="ru")dict=Ru;
            else dict=En;
        }
    ...
    public class MainForm : Form
    {
        ...
        private void InitUI()
        {
            ...
            tick.Tick+=new EventHandler(delegate(object s,EventArgs e)
            {
                ...
                trafficText=FormatRate(kbIn)+" \u25BC   "+FormatRate(kbOut)+" \u25B2   "
                    +string.Format(L.T("conns_fmt"),statConns);
        ...
        private void StatusPaint(object sender,PaintEventArgs e)
        {
            ...
            TextRenderer.DrawText(g,st,new Font("Verdana",8f,FontStyle.Bold),
                new Rectangle(30,0,150,28),on?Color.FromArgb(30,120,50):t.Text,
                TextFormatFlags.VerticalCenter|TextFormatFlags.Left);

            TextRenderer.DrawText(g,trafficText,new Font("Verdana",8f),
            ...
            if(upActive>0&&upTotal>0)
            {
                ...
                TextRenderer.DrawText(g,txt,new Font("Verdana",7.5f,FontStyle.Bold),
        ...
        private void StartServer()
        {
            ...
            catch
            {
                ...
                { MessageBox.Show(string.Format(L.T("msg_start_fail"),ex2.Message),"GoldShare"); return; }
            }
            running=true;
        ...
        private void HandleZip(HttpListenerContext ctx)
        {
            ...
            List<long[]> central=new List<long[]>();
            List<byte[]> centralNames=new List<byte[]>();
            for(int i=0;i<items.Count;i++)
            {
                string[] it=items[i];
                byte[] data;
                using(FileStream fs=new FileStream(it[2],FileMode.Open,FileAccess.Read,FileShare.Read))
                {
                    data=new byte[fs.Length];
                    int done=0;
                    while(done<data.Length)
                    {
                        int r=fs.Read(data,done,data.Length-done);
                        if(r<=0) break;
                        done+=r;
                    }
                }
                uint crc=Crc32(data,0,data.Length);
                byte[] nb=Encoding.UTF8.GetBytes(it[0]+"/"+it[1]);
                long off=ms.Position;
                bw.Write((int)0x04034b50); bw.Write((short)20); bw.Write((short)0); bw.Write((short)0);
                bw.Write((short)0); bw.Write((short)0);
                bw.Write(crc); bw.Write((int)data.Length); bw.Write((int)data.Length);
                bw.Write((short)nb.Length); bw.Write((short)0);
                bw.Write(nb); bw.Write(data);
                central.Add(new long[]{crc,data.Length,off});
                centralNames.Add(nb);
            }
            ...
            for(int i=0;i<central.Count;i++)
            {
                ...
                bw.Write((uint)e[0]); bw.Write((int)e[1]); bw.Write((int)e[1]);
                bw.Write((short)centralNames[i].Length);
                bw.Write((short)0); bw.Write((short)0); bw.Write((short)0); bw.Write((short)0);
                bw.Write((int)0); bw.Write((int)e[2]); bw.Write(centralNames[i]);
            }
            long cdSize=ms.Position-cdStart;
            bw.Write((int)0x06054b50); bw.Write((short)0); bw.Write((short)0);
            bw.Write((short)central.Count); bw.Write((short)central.Count);
            bw.Write((int)cdSize); bw.Write((int)cdStart); bw.Write((short)0);
            bw.Flush();

            Log(string.Format(L.T("msg_zip_ok"),items.Count,FormatSize(ms.Length)));
            ctx.Response.AddHeader("Content-Disposition","attachment; filename=\"GoldShare-Files.zip\"");
            WriteBytes(ctx,ms.ToArray(),200,"application/zip");
        }
        ...
        // --------------------------------------------------------------------
        //  UPLOAD (multipart parser + GUI progress + beep)
        // --------------------------------------------------------------------
        private void HandleUpload(HttpListenerContext ctx)
        {
            ...
            try
            {
                ...
                while(pos>=0)
                {
                    ...
                    if(segStart+1<body.Length&&body[segStart]==(byte)'-'&&body[segStart+1]==(byte)'-') break;
                    if(segStart+1<body.Length&&body[segStart]==13&&body[segStart+1]==10) segStart+=2;
                    int dataEnd=next-2;
                    if(dataEnd>segStart)
                    {
