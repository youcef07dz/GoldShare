    ...
    public class MainForm : Form
    {
        ...
        private void InitUI()
        {
            ...
            tick.Tick+=new EventHandler(delegate(object s,EventArgs e)
            {
                ...
                if(running)
                {
                    int phase=Environment.TickCount/500;
        ...
        private static IntPtr cachedHandle=IntPtr.Zero;
        private static void TrimWorkingSet()
        {
            try
            {
                if(cachedHandle==IntPtr.Zero)
                    cachedHandle=System.Diagnostics.Process.GetCurrentProcess().Handle;
                SetProcessWorkingSetSize(cachedHandle,new IntPtr(-1),new IntPtr(-1));
            } catch { }
        }
